module pyth_v1::batch_price_attestation {
    use std::vector::{Self};
    use sui::clock::{Self, Clock};

    use pyth_v1::price_feed::{Self};
    use pyth_v1::price_info::{Self, PriceInfo};
    use pyth_v1::price_identifier::{Self};
    use pyth_v1::price_status;
    ...
    fun deserialize_price_info(cur: &mut Cursor<u8>, clock: &Clock): PriceInfo {
        ...
        let current_price = pyth_v1::price::new(price, conf, expo, publish_time);
        if (status != price_status::new_trading()) {
            current_price = pyth_v1::price::new(prev_price, prev_conf, expo, prev_publish_time);
        };
        ...
        price_info::new_price_info(
            ...
            price_feed::new(
                price_identifier,
                current_price,
                pyth_v1::price::new(ema_price, ema_conf, expo, ema_timestamp),
            )
        )
    }
