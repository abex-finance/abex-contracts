...
for c_coin in ${c_coins[*]}; do
       ...
       # remove collateral to symbol
       remove_log=`sui client --client.config $config \
              call --gas-budget $gas_budget \
                     --package $package \
                     --module market \
                     --function remove_collateral_from_symbol \
                     --type-args $package::alp::ALP ${c_coin_module} ${i_coin_module} $package::market::$direction \
                     --args ${admin_cap} $market`
       echo "${remove_log}"

       ok=`echo "${remove_log}" | grep "Status : Success"`
       if [ -n "$ok" ]; then
              symbol="${direction}_${i_coin}"
              declare -l symbol=$symbol
              json_content=`jq "(.abex_core.symbols.$symbol.supported_collaterals) |= map(select(. != \"${c_coin}\"))" $deployments`
